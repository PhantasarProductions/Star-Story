...
function AblSpecialEffect.Randomizer(ag,ai,tg,ti,act)
  local ftag = FighterTag(tg,ti)
  local pointstats = mysplit(RPGChar.PointsFields(ftag),";")
  local ret
  for s in each(pointstats) do
    ret = P_Rand(RPGChar.Points(ftag,s))
    end
return ret
end
